...
5. Serial TTY devices. Typically, serial devices on Linux and macOS are reserved for the root user and those in the `dialout` group. You'll know it if the computer complains that you don't have the approrpriate privileges to use `/dev/ttyUSB0` (or other). If can add yourself by typing `sudo usermod -a -G dialout <username>`, where `<username>` is your username. Also note that not all USB serial devices can handle the hardware handshake that is required by some radios. You may want to check your radio and the serial device's specs to make sure they are compatible in that regard.
